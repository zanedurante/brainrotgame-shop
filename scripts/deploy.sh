...
tar -czf "$archive" index.html assets/play-counts.js counter/server.py counter/export_counts.py ops/gameslop-plays.service ops/brainrotgame.caddy ops/install-counter.sh ops/gameslop-plays-export.cron ops/install-exports.sh
...
ssh "$HOST" "tar -xzf '$stage/release.tar.gz' -C '$stage' && EXPECTED_PORTAL_SHA256='$expected' bash '$stage/ops/install-counter.sh' '$stage' && bash '$stage/ops/install-exports.sh' '$stage'"
